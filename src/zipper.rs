...
pub use crate::product_zipper::{OneFactor, ProductZipper, ProductZipperG, ZipperProduct};
pub use crate::restrict_zipper::RestrictZipper;
pub use crate::subtract_zipper::SubtractZipper;
...
pub trait Zipper {
    /// Returns `true` if the zipper's focus is on a path within the trie.
    ///
    /// A missing path proves that no value exists at or below the focus. An
    /// existing path does not prove that a value exists there or below it.
    /// Write zippers and maps can create dangling paths with no value.
    /// Use [`Zipper::is_val`], [`ZipperValues::val`], or value iteration when
    /// the caller needs to observe values.
    ...
    #[deprecated(note = "use Zipper::is_val")]
...
pub trait ZipperValues<V> {
    ...
    #[deprecated(note = "use ZipperValues::val")]
...
pub trait ZipperForking<V> {
    ...
    ///
    /// Design note: the main uses of this method are to construct a child zipper with a different root
    ...
    ZipperValues<V>
{
    ...
    ///
    /// The value at the zipper focus is copied as the returned map's root value.
    ///
...
/// Materializes the pathspace observed below a zipper's current focus into a new [`PathMap`].
///
/// This walks only the public zipper observation interface. It is intended for
/// virtual zippers that can describe a residual pathspace but cannot expose a
/// concrete subtrie in constant time.
///
/// Returned paths are relative to the starting focus. If `zipper` is focused at
/// `b"prefix/"`, a value observed at `b"prefix/value"` is written to
/// `b"value"` in the returned map.
pub fn materialize_zipper<V, A, Z>(zipper: Z, alloc: A) -> PathMap<V, A>
where
    ...
    Z: ZipperMoving + ZipperValues<V>,
{
    ...
    crate::counters::record_observer_materialization();
    let mut zipper = zipper;
    let mut map = PathMap::new_in(alloc);
    let mut relative_path = Vec::new();
    materialize_zipper_focus(&mut zipper, &mut map, &mut relative_path);
    map
}

/// Resets a zipper to its root, then materializes the observed root residual.
pub fn materialize_zipper_from_root<V, A, Z>(mut zipper: Z, alloc: A) -> PathMap<V, A>
where
    V: Clone + Send + Sync + Unpin,
    A: Allocator,
    Z: ZipperMoving + ZipperValues<V>,
{
    zipper.reset();
    materialize_zipper(zipper, alloc)
}

fn materialize_zipper_focus<V, A, Z>(
    zipper: &mut Z,
    map: &mut PathMap<V, A>,
    relative_path: &mut Vec<u8>,
) where
    V: Clone + Send + Sync + Unpin,
    A: Allocator,
    Z: ZipperMoving + ZipperValues<V>,
{
    let path_exists = zipper.path_exists();
    let value = zipper.val().cloned();
    if path_exists || value.is_some() {
        if let Some(value) = value {
            map.set_val_at(&*relative_path, value);
            #[cfg(feature = "counters")]
            crate::counters::record_observer_materialized_path(true);
        } else if !relative_path.is_empty() {
            map.create_path(&*relative_path);
            #[cfg(feature = "counters")]
            crate::counters::record_observer_materialized_path(false);
        }
    }

    for child_byte in zipper.child_mask().iter() {
        zipper.descend_to_byte(child_byte);
        relative_path.push(child_byte);
        if zipper.path_exists() {
            materialize_zipper_focus(zipper, map, relative_path);
        }
        relative_path.pop();
...
#[cfg(test)]
mod materialize_zipper_tests {
    use super::{materialize_zipper, Zipper, ZipperMoving, ZipperValues};
    use crate::alloc::global_alloc;
    use crate::utils::ByteMask;
    use crate::PathMap;

    struct MaskOnlyZipper {
        path: Vec<u8>,
        low: u8,
        mid: u8,
        high: u8,
    ...
    impl Default for MaskOnlyZipper {
        fn default() -> Self {
            Self {
                path: Vec::new(),
                low: 10,
    ...
    impl MaskOnlyZipper {
        fn is_child_path(&self) -> bool {
            matches!(self.path.as_slice(), [1] | [128] | [255])
        }
    }

    impl Zipper for MaskOnlyZipper {
        fn path_exists(&self) -> bool {
            self.path.is_empty() || self.is_child_path()
        }
        ...
        fn child_count(&self) -> usize {
            panic!("materialize_zipper should enumerate child_mask bytes directly")
        }

        fn child_mask(&self) -> ByteMask {
            if self.path.is_empty() {
                ByteMask::from_iter([1u8, 128, 255])
            } else {
                ByteMask::EMPTY
            }
        }
    ...
    impl ZipperValues<u8> for MaskOnlyZipper {
        fn val(&self) -> Option<&u8> {
    ...
    impl ZipperMoving for MaskOnlyZipper {
        fn path(&self) -> &[u8] {
            &self.path
        }
        ...
        fn descend_indexed_byte(&mut self, _child_idx: usize) -> bool {
            panic!("materialize_zipper should not use indexed child descent")
        }

        fn ascend(&mut self, steps: usize) -> bool {
            let original_len = self.path.len();
            self.path.truncate(original_len.saturating_sub(steps));
            self.path.len() != original_len
        }

        fn ascend_until(&mut self) -> bool {
            let moved = !self.path.is_empty();
            self.path.clear();
            moved
        }
    ...
    #[test]
    fn materialize_zipper_enumerates_sparse_child_mask_bytes() {
        let materialized = materialize_zipper(MaskOnlyZipper::default(), global_alloc());

        assert_eq!(materialized.get_val_at([1]), Some(&10));
        assert_eq!(materialized.get_val_at([128]), Some(&20));
        assert_eq!(materialized.get_val_at([255]), Some(&30));
        assert_eq!(materialized.val_count(), 3);
    }

    #[test]
    fn materialize_zipper_keeps_paths_relative_to_starting_focus() {
        let mut source = PathMap::new();
        source.set_val_at(b"prefix/value", 1);
        source.create_path(b"prefix/dangling");
        source.set_val_at(b"outside", 2);

        let materialized =
            materialize_zipper(source.read_zipper_at_path(b"prefix/"), global_alloc());

        assert_eq!(materialized.get_val_at(b"value"), Some(&1));
        assert!(materialized.path_exists_at(b"dangling"));
        assert!(!materialized.path_exists_at(b"prefix/value"));
        assert!(!materialized.path_exists_at(b"outside"));
    }
}
...
pub trait ZipperMoving: Zipper {
    ...
    ///
    /// Cost: implementations may walk every value below the focus.
    fn val_count(&self) -> usize;
    ...
    fn move_to_path<K: AsRef<[u8]>>(&mut self, path: K) -> usize {
        ...
        if overlap == 0 {
            // This heuristic can be fine-tuned for speed; the behavior of the two branches is equivalent.
            self.reset();
    ...
    /// the path.
    ///
    /// The default implementation descends one byte at a time. Implementors with direct access to
    /// node-local value positions may provide a faster implementation.
    ...
    #[deprecated(note = "use ZipperMoving::descend_to_val")]
    ...
    ///
    /// Index stability: the branch represented by a given index is not guaranteed to be stable across
    /// modifications to the trie. Use this only as part of a directed traversal operation. Do not store
    /// index-based paths as locations within the trie.
    ...
    #[deprecated(note = "use ZipperMoving::descend_indexed_byte")]
...
pub trait ZipperReadOnlyValues<'a, V>: ZipperValues<V> {
    ...
    #[deprecated(note = "use ZipperReadOnlyValues::get_val")]
...
pub trait ZipperReadOnlyIteration<'a, V>: ZipperReadOnlyValues<'a, V> + ZipperIteration {
    /// Moves to the first value reachable from the zipper's root and returns it.
    fn to_first_get_val(&mut self) -> Option<&'a V> {
        if self.to_first_val() {
    ...
    #[deprecated(note = "use ZipperReadOnlyIteration::to_next_get_val")]
...
{
    /// See [ZipperReadOnlyIteration::to_first_get_val] and [`witness`](ZipperReadOnlyConditionalValues::witness)
    fn to_first_get_val_with_witness<'w>(&mut self, witness: &'w Self::WitnessT) -> Option<&'w V>
    where
        'a: 'w,
    {
        if self.to_first_val() {
...
#[doc(hidden)]
#[derive(Debug)]
pub struct OpaqueAbstractNodeRef<'trie, V: Clone + Send + Sync, A: Allocator>(
    pub(crate) AbstractNodeRef<'trie, V, A>,
);
...
impl<'a, V: Clone + Send + Sync, A: Allocator> OpaqueAbstractNodeRef<'a, V, A> {
    ...
    #[cfg(feature = "viz")]
    pub(crate) fn borrow(&self) -> Option<&TrieNodeODRc<V, A>> {
        self.0.borrow()
    }
    pub(crate) fn into_option(self) -> Option<TrieNodeODRc<V, A>> {
        self.0.into_option()
    }
    pub(crate) fn as_tagged(&self) -> TaggedNodeRef<'_, V, A> {
        self.0.as_tagged()
    }
    pub(crate) fn try_as_tagged(&self) -> Option<TaggedNodeRef<'_, V, A>> {
        self.0.try_as_tagged()
    }
}
...
#[doc(hidden)]
#[derive(Debug)]
pub struct OpaqueTrieNodeRef<'trie, V: Clone + Send + Sync, A: Allocator>(
    pub(crate) &'trie TrieNodeODRc<V, A>,
);
    ...
    ZipperValues<V>
{
    ...
    ///
    /// `try_borrow_focus` and `get_focus` serve different internal callers.
    ///
...
pub trait ZipperIteration: ZipperMoving {
    /// Moves the zipper to the first value reachable from its root.
    ///
    /// If the root holds a value, this method stops there. Otherwise it advances in the same depth-first
    /// order as [`to_next_val`](Self::to_next_val).
    fn to_first_val(&mut self) -> bool {
        self.reset();
        if self.is_val() {
            true
        } else {
    ...
    ///
    /// Cost: this is not constant time, and may be as bad as `order n` with respect to the paths
    /// below the zipper's focus. A typical cost is `order log n` or better.
    ///
...
pub trait ZipperConcrete {
    ...
    /// more distinct paths
    ///
    /// The `shared` property applies only to the current focus. It is not transitive to descendants.
    ///
    /// ```text
    /// path a --> shared node S --> child x
    /// path b ---------^
    /// ```
    ///
    /// A zipper focused at `a` or `b` may report `true` because both paths reach `S`. A zipper
    /// focused at `a/x` may still report `false` if `x` is not itself shared.
    ///
    /// Use this only for optimizations. Correctness must never depend on it. Sharing can change when
    /// a subtrie is copied for thread isolation or when the internal representation changes.
...
pub(crate) mod zipper_priv {
    ...
    pub trait ZipperReadOnlyPriv<'a, V: Clone + Send + Sync, A: Allocator> {
        ...
        ///
        /// Invariant: the original zipper must stay alive while the returned core is used.
        ///
        /// This method does not consume the zipper because the original keeps the tracker, or perhaps a backing map,
        /// active while the core is in use.
        ///
...
macro_rules! zipper_impl_lens {
    (ZipperIteration $s: ident => $e:expr) => {
        fn to_first_val(&mut $s) -> bool { $e.to_first_val() }
    ...
    (ZipperReadOnlyIteration $s: ident => $e:expr) => {
        fn to_first_get_val(&mut $s) -> Option<&'a V> { $e.to_first_get_val() }
        fn to_next_get_val(&mut $s) -> Option<&'a V> { $e.to_next_get_val() }
    };
    (ZipperReadOnlyConditionalIteration $s: ident => $e:expr) => {
        fn to_first_get_val_with_witness<'w>(&mut $s, witness: &'w Self::WitnessT) -> Option<&'w V> where 'a: 'w { $e.to_first_get_val_with_witness(witness) }
...
// `ReadZipperTracked` intentionally does not implement `IntoIterator`.
// The standard iterator item type would need to carry value references that outlive each call to
// `next`, but tracked read zippers require a witness to extend those borrows while the cursor moves.
// Use `fork_read_zipper` to create an untracked iterator when the parent permissions cover the walk.
...
///
/// The backing map is retained because root values still live outside trie nodes. The boxed
/// `ReadZipperCore` borrows from that map while the owned zipper is alive.
...
impl<V: 'static + Clone + Send + Sync + Unpin, A: Allocator> ReadZipperOwned<V, A> {
    ...
    pub(crate) fn new_with_map<K: AsRef<[u8]>>(map: PathMap<V, A>, path: K) -> Self {
        ...
        let root_val = root_ref
            .as_tagged()
            .node_get_root_val()
            .or_else(|| Option::as_ref(unsafe { &*(*map).root_val.get() }));
        let core = ReadZipperCore::new_with_node_and_cloned_path_in(
            root_ref,
            true,
            path,
...
pub(crate) mod read_zipper_core {
    ...
    /// Narrow root-storage helper for `ReadZipperCore`.
    ///
    /// Keep this separate from `AbstractNodeRef`. `AbstractNodeRef` is observer
    /// output and can represent dynamic or tiny node views. `root_node` must
    /// hold either no root, a borrowed whole `TrieNodeODRc`, or an owned whole
    /// `TrieNodeODRc` that keeps self-referential focus pointers valid for the
    /// lifetime of the zipper. Merging the two types belongs to a broader
    /// read-zipper storage redesign.
    #[derive(Clone, Debug)]
        ...
        fn get_focus(&self) -> OpaqueAbstractNodeRef<'_, V, A> {
            ...
            let (focus_node, node_key) = if node_key.len() == 0 {
                match self.ancestors.last() {
                    ...
                    None => {
                        return OpaqueAbstractNodeRef(AbstractNodeRef::BorrowedRc(
    ...
    >(
        ...
        if !zipper.is_shared() || !key.is_empty() || value.is_some() {
            // Values associated with nodes that can be shared are stored outside the node.
            // One focus address can therefore correspond to distinct points with distinct
            // values. Nodes with focus values are not safe cache keys.
            // Design context:
            // PR 8 review thread r2005555762
        ...
        for ReadZipperCore<'trie, '_, V, A>
    {
        fn to_first_val(&mut self) -> bool {
            self.reset();
            self.is_val_internal() || self.to_next_val()
        }
        fn to_next_val(&mut self) -> bool {
    ...
    for ReadZipperIter<'a, '_, V, A>
{
    ...
    fn next(&mut self) -> Option<(Vec<u8>, &'a V)> {
        ...
        if let Some(zipper) = &mut self.zipper {
            //SAFETY: we only allow ReadZipperUntracked to become a `ReadZipperIter`
            if let Some(val) = unsafe { zipper.to_next_get_val() } {
                return Some((zipper.path().to_vec(), val));
            }
        }
        self.zipper = None;
...
mod tests {
    ...
    #[cfg(all(target_arch = "x86_64", not(miri), feature = "slim_ptrs"))]
    static_assertions::assert_eq_size!(
        read_zipper_core::ReadZipperCore<'static, 'static, (), crate::alloc::GlobalAlloc>,
        [u8; 128]
    );

    #[cfg(all(target_arch = "x86_64", not(miri), not(feature = "slim_ptrs")))]
    static_assertions::assert_eq_size!(
        read_zipper_core::ReadZipperCore<'static, 'static, (), crate::alloc::GlobalAlloc>,
        [u8; 136]
    );
    ...
    #[test]
    fn read_zipper_to_first_val_starts_at_root_value() {
        let mut map = PathMap::<usize>::new();
        map.set_val_at([], 0);
        map.set_val_at(b"a", 1);
        map.set_val_at(b"in", 2);
        map.set_val_at(b"in/a", 3);

        let mut root_z = map.read_zipper();
        assert!(root_z.to_first_val());
        assert_eq!(root_z.path(), b"");
        assert_eq!(root_z.get_val(), Some(&0));
        assert!(root_z.to_next_val());
        assert_eq!(root_z.path(), b"a");
        assert_eq!(root_z.get_val(), Some(&1));

        let mut sub_z = map.read_zipper_at_path(b"in");
        assert!(sub_z.to_first_val());
        assert_eq!(sub_z.path(), b"");
        assert_eq!(sub_z.get_val(), Some(&2));
        assert!(sub_z.to_next_val());
        assert_eq!(sub_z.path(), b"/a");
        assert_eq!(sub_z.get_val(), Some(&3));
    }
    ...
    fn read_zipper_is_shared_test1() {
        ...
        let l2_keys = ["X-top0", "X-top1", "X-top2", "X-top3"];
    ...
    fn read_zipper_is_shared_test2() {
        let l0_keys = ["steam", "steamboat"];
        let l1_keys = ["X0", "X1", "X2"];
