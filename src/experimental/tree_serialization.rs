...
pub fn serialize_fork<V: TrieValue, RZ: Catamorphism<V>, F: FnMut(usize, &[u8], &V) -> ()>(
    ...
    unsafe {
        thread_local! {
            static WRITTEN: UnsafeCell<usize> = const { UnsafeCell::new(0) }
        }
        WRITTEN.with(|w| {
...
mod tests {
    use crate::PathMap;
    use crate::experimental::tree_serialization::{deserialize_fork, serialize_fork};
