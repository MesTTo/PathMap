...
use crate::dense_byte_node::{ByteNode, CellCoFree, CoFree, DenseByteNode, OrdinaryCoFree};
use crate::ring::*;
...
// The live slim-pointer layout uses all 64 bytes of one cache line. The raw
// pointer fallback keeps room for the allocation header, so its inline key
// budget stays smaller.
// The header length bits require KEY_BYTES_CNT <= 63.
#[cfg(feature = "slim_ptrs")]
...
impl<V: Clone + Send + Sync, A: Allocator> Drop for LineListNode<V, A> {
    fn drop(&mut self) {
        // Design note: The straightforward recursive implementation hits a stack overflow with some very
        ...
        if let Some((onward_key, onward_node)) = follow_path(other, path) {
            if self.is_child_ptr::<SLOT>() {
                ...
                let difference = if onward_key.len() == 0 {
                    // COW-identity skip: if self's child and the aligned node in
                    // `other` are the same node in memory, every path under self is
                    // also under other, so the difference for this subtree is empty.
                    // Same precedent as `TrieNodeODRc::psubtract`'s `ptr_eq` guard,
                    // restored here where the tagged-ref recursion would otherwise
                    // walk the whole shared subtree. Sound for any two live nodes:
                    // equal `shared_node_id` is equal memory address is identical
                    // contents.
                    if !self_onward_link.node_is_empty()
                        && self_onward_link.shared_node_id() == onward_node.shared_node_id()
                    {
                        return AlgebraicResult::None;
                    }
                    ...
                    match onward_node.get_node_at_key(onward_key).into_option() {
                        Some(other_onward_node) => {
                            let other_onward_node = other_onward_node.as_tagged();
                            if self_onward_link.shared_node_id()
                                == other_onward_node.shared_node_id()
                            {
                                return AlgebraicResult::None;
                            }
                            self_onward_link.psubtract_dyn(other_onward_node)
                        }
                        None => return AlgebraicResult::Identity(SELF_IDENT),
                    }
                };
                debug_assert!(difference
                    .as_ref()
                    .map(|node| node
                        .as_tagged()
                        .as_list()
            ...
            (true, true) => {
                ...
                return match a_child.pjoin(b_child) {
                    AlgebraicResult::Element(new_child) => {
                        AlgebraicResult::Element((a_key, ValOrChild::Child(new_child)))
                    }
...
mod tests {
    ...
    #[cfg(all(feature = "slim_ptrs", target_arch = "x86_64", not(miri)))]
    static_assertions::assert_eq_size!(LineListNode<[u8; 1024], GlobalAlloc>, [u8; 64]);

    #[cfg(all(not(feature = "slim_ptrs"), target_arch = "x86_64", not(miri)))]
    static_assertions::assert_eq_size!(LineListNode<[u8; 1024], GlobalAlloc>, [u8; 48]);
