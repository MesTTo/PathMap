...
impl<'a, V: Clone + Send + Sync, A: Allocator> TinyRefNode<'a, V, A> {
    ...
    pub fn into_bridge_node(&self) -> Option<crate::bridge_node::BridgeNode<V, A>> {
        ...
        Some(crate::bridge_node::BridgeNode::new(
            self.key(),
            is_child,
            payload,
        ))
...
impl<'a, V: Clone + Send + Sync, A: Allocator> TrieNode<V, A> for TinyRefNode<'a, V, A> {
    ...
    fn next_items(
        &self,
        _token: IterToken,
    ) -> (IterToken, &'a [u8], Option<&TrieNodeODRc<V, A>>, Option<&V>) {
        unreachable!()
    }
    ...
    where
        V: Lattice,
    {
        // TinyRefNode is a borrowed singleton view. Reuse LineListNode's compressed-key
        // algebra instead of keeping a second bespoke implementation in sync.
    ...
    where
        V: Lattice,
    {
        // Reuse LineListNode's compressed-key algebra. This path is for transient borrowed views.
...
mod tests {
    ...
    static_assertions::assert_eq_size!(TinyRefNode<'static, (), GlobalAlloc>, [u8; 16]);
