...
#[cfg(all(test, target_arch = "x86_64", not(miri), feature = "slim_ptrs"))]
static_assertions::assert_eq_size!(DenseByteNode<(), crate::alloc::GlobalAlloc>, [u8; 64]);

#[cfg(all(test, target_arch = "x86_64", not(miri), feature = "slim_ptrs"))]
static_assertions::assert_eq_size!(CellByteNode<(), crate::alloc::GlobalAlloc>, [u8; 64]);

#[cfg(all(test, target_arch = "x86_64", not(miri), not(feature = "slim_ptrs")))]
static_assertions::assert_eq_size!(DenseByteNode<(), crate::alloc::GlobalAlloc>, [u8; 56]);

#[cfg(all(test, target_arch = "x86_64", not(miri), not(feature = "slim_ptrs")))]
static_assertions::assert_eq_size!(CellByteNode<(), crate::alloc::GlobalAlloc>, [u8; 56]);
        ...
        if self.mask.test_bit(k) {
            ...
            match cf.rec_mut() {
                Some(existing_node) => {
                    ...
                    } else if existing_node.is_empty() {
                        *existing_node = node;
    ...
    Self: TrieNodeDowncast<V, A>,
{
                ...
                if let Some(self_child) = cf.rec() {
                    ...
                    if !self_child_tagged.node_is_empty() {
                        ...
                        match other_child.try_as_tagged() {
                            // COW-identity skip: same node in memory => empty
                            // difference for this subtree (mirrors the `ptr_eq`
                            // guard in `TrieNodeODRc::psubtract`). Leaves `new_cf`'s
                            // rec unset, which drops this child from the result.
                            Some(other_child)
                                if self_child_tagged.shared_node_id()
                                    == other_child.shared_node_id() =>
                            {
                                is_identity = false;
                            }
                            Some(other_child) => {
    ...
    ///
    /// The list node is borrowed, so payloads are cloned before insertion. An
    /// owned-list variant could move payloads instead, but it would need a
    /// separate call path for callers that retain the source node.
    ...
    ByteNode<Cf, A>: TrieNodeDowncast<V, A>,
{
    ...
    fn node_get_payloads<'node, 'res>(
        ...
        // Design note: this function appears overly complicated primarily because it needs to track
    ...
    fn node_set_branch(
        ...
        #[cfg(not(feature = "all_dense_nodes"))]
        {
            ...
            if key.len() > 1 {
                ...
                {
                    let child =
                        crate::bridge_node::BridgeNode::new(&key[1..], true, new_node.into());
                    self.set_child(key[0], TrieNodeODRc::new(child));
                }
                Ok(true)
            } else {
    ...
    fn node_val_count(&self, cache: &mut HashMap<u64, usize>) -> usize {
        // Design note: these two implementations do the same thing but with a slightly different ordering of
    ...
    fn nth_child_from_key(
        ...
        //
        // However, this capability may be used again, so I made "FORWARD" a const instead of an argument.
        //
        ...
        V: Lattice,
    {
        match self.values.len() {
            0 => None,
            1 => {
                // Do not swap the node itself with its first child. That looks like an optimization, but it
                // would leak memory because the other node would then hold an Rc to itself.
                match self.values.pop().unwrap().into_rec() {
                    Some(mut child) if !child.is_empty() => {
                        if byte_cnt > 1 {
    ...
    > HeteroLattice<OtherCf> for Cf
{
    fn pjoin(&self, other: &OtherCf) -> AlgebraicResult<Self> {
        let self_dangling = !self.has_rec() && !self.has_val();
        let other_dangling = !other.has_rec() && !other.has_val();
        match (self_dangling, other_dangling) {
            (true, true) => return AlgebraicResult::Identity(SELF_IDENT | COUNTER_IDENT),
            (true, false) => return AlgebraicResult::Identity(COUNTER_IDENT),
            (false, true) => return AlgebraicResult::Identity(SELF_IDENT),
            (false, false) => {}
        }
