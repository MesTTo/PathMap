...
## Structural Sharing
Storage inside `PathMap` makes use of references that can point to data within the same map or across multiple maps.

Here is a conceptual example sharing subtrie structure.  This map contains two separate hand structure instances, even though they are the same in memory.

The map contains both of the paths:  
`1-Hand.0-Left.Finger.3-Ring`  
`1-Hand.1-Right.Finger.3-Ring`

<!-- ![4x4_sharing_pathmap](images/1.00.00_sharing.svg) -->
<p align="center">
  <object type="image/svg+xml" data="images/1.00.00_sharing.svg" style="width:80%;"></object>
</p>

This ability means that it is possible to construct a map with far more values than could fit in memory if they were all stored individually.

Below is a diagram representating a map that contains every possible 4-byte-long path composed of the characters `['a', 'b', 'c', 'd']`.  As you can see in the graph, 16 path bytes are stored within the structure.  Without structural sharing, it would be necessary to store 256 bytes (`4^4`) to represent the same tree.

<!-- ![4x4_sharing_pathmap](images/1.00.00_heavy_sharing.svg) -->
<p align="center">
  <object type="image/svg+xml" data="images/1.00.00_heavy_sharing.svg" style="width:50%;"></object>
</p>
