...
use pathmap::zipper::{
    ReadZipperUntracked, WriteZipperUntracked, Zipper, ZipperMoving, ZipperReadOnlyValues,
    ZipperWriting,
};
...
fn run(bencher: Bencher, stage: &str) {
    ...
    if points.point_layout().has_attribute(&POSITION_3D) {
        ...
        positions
            .clone()
            .into_iter()
            .for_each(|point| rbbox = AABB::extend_with_point(&rbbox, &point.into()));
        ...
        if stage == "build" {
            return bencher.bench_local(|| {
                let mut tree = BTMOctree::new(rbbox);
                positions
                    .clone()
                    .into_iter()
                    .for_each(|point| tree.insert(point.into()));
                tree
            });
        }
        let mut tree = BTMOctree::new(rbbox);
        ...
        if stage == "query" {
            return bencher.bench_local(|| {
                let mut contained = 0;
                tree.range_query(first_octant, &mut |_| {
                    contained += 1;
                    true
                });
        ...
        if stage == "knn" {
            return bencher.bench_local(|| {
                ...
                let reference = vec![
                    ...
                    &[-4.12, 1.2, -1.87],
                    &[-4.12, 1.2, -1.87],
                    &[-4.09, 1.08, -2.0100000000000002],
                    &[-4.09, 1.08, -2.0100000000000002],
                    &[-4.12, 1.19, -1.87],
                    &[-4.09, 1.08, -2.0],
                    &[-4.1, 1.08, -2.0100000000000002],
                    &[-4.09, 1.08, -1.99],
                    &[-4.13, 1.17, -1.8900000000000001],
                ];
                for (top, rtop) in topk.iter().zip(reference.iter()) {
                    for (co, rco) in top.iter().zip(rtop.iter()) {
                        assert_eq!(co, rco)
                    }
                }
                topk
...
impl Octree {
    ...
    fn insert_rec(node: &mut Node, p: Point3<f64>, bounds: AABB<f64>, depth: usize) {
        match node {
            ...
            Node::Branch { children } => {
                ...
                Self::insert_rec(children[idx].as_mut().unwrap(), p, child_bounds, depth + 1);
            }
        }
    }
    ...
    pub fn range_query<F: FnMut(Point3<f64>) -> bool>(
        &self,
        query: AABB<f64>,
        mut f: &mut F,
    ) -> bool {
    ...
    fn query_rec<F: FnMut(Point3<f64>) -> bool>(
        node: &Node,
        bounds: AABB<f64>,
        query: &AABB<f64>,
        f: &mut F,
            ...
            Node::Branch { children } => {
                for idx in 0..8 {
                    if let Some(child) = &children[idx] {
                        let child_bounds = Self::child(&bounds, idx);
                        if !Self::query_rec(child, child_bounds, query, f) {
                            return false;
                        }
                    }
                }
    ...
    fn try_add_candidate(best: &mut Vec<(f64, Point3<f64>)>, k: usize, dist2: f64, p: Point3<f64>) {
        ...
        let pos =
            best.binary_search_by(|a| a.0.partial_cmp(&dist2).unwrap_or(std::cmp::Ordering::Equal));
...
impl BTMOctree {
    ...
    fn insert_rec(
        wz: &mut WriteZipperUntracked<Vec<Point3<f64>>>,
        p: Point3<f64>,
        bounds: AABB<f64>,
        depth: usize,
    ) {
    ...
    fn query_rec<F: FnMut(Point3<f64>) -> bool>(
        rz: &mut ReadZipperUntracked<Vec<Point3<f64>>>,
        bounds: AABB<f64>,
        query: &AABB<f64>,
        f: &mut F,
    ) -> bool {
        ...
        for idx in rz.child_mask().iter() {
            ...
            if !Self::query_rec(rz, child_bounds, query, f) {
                return false;
            }
            rz.ascend_byte();
        }
