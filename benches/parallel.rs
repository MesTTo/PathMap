...
fn main() {
    let divan = Divan::from_args().sample_count(8);
...
fn parallel_read_zipper_get(bencher: Bencher, (elements, thread_cnt): (usize, &str)) {
    ...
    thread::scope(|scope| {
        let mut zipper_senders: Vec<mpsc::Sender<ReadZipperUntracked<'_, '_, usize>>> =
...
fn parallel_insert(bencher: Bencher, (elements, thread_cnt): (usize, &str)) {
    ...
    thread::scope(|scope| {
        let mut zipper_senders: Vec<mpsc::Sender<WriteZipperTracked<'_, '_, usize>>> =
            Vec::with_capacity(thread_cnt);
        let mut signal_receivers: Vec<mpsc::Receiver<bool>> = Vec::with_capacity(thread_cnt);
        ...
        bencher.with_inputs(|| {}).bench_local_values(|()| {
            if thread_cnt > 0 {
                ...
                let mut zipper =
                    unsafe { zipper_head.write_zipper_at_exclusive_path_unchecked(&[0]) };
                for i in 0..elements {
                    zipper.move_to_path(prefix_key(&(i as u64)));
                    zipper.set_val(i);
                }
        ...
        bencher
            ...
            .bench_local_values(|()| {
                if thread_cnt > 0 {
                    ...
                    //No-thread case, to measure overhead of sync vs. 1-thread case
                    let mut writer_z = unsafe {
                        zipper_head.write_zipper_at_exclusive_path_unchecked(&[b'o', b'u', b't', 0])
                    };
                    let mut reader_z =
                        unsafe { zipper_head.read_zipper_at_path_unchecked(&[b'i', b'n', 0]) };
                    let witness = reader_z.witness();
                    for i in 0..elements {
