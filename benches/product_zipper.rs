...
fn main() {
    ...
    let divan = Divan::from_args().sample_count(16);
...
fn introspecting_pathmap_pathmap(bencher: Bencher) {
    use pathmap::{PathMap, morphisms::Catamorphism, zipper::ProductZipper};
    let mut sink = 0;
    let pairs = read_data();
    let map1 = PathMap::from_iter(
        pairs
            .iter()
            .take(CUT_CITIES_LIST)
...
fn generic_pathmap_pathmap(bencher: Bencher) {
    use pathmap::{PathMap, morphisms::Catamorphism, zipper::ProductZipperG};
    let mut sink = 0;
    let pairs = read_data();
    let map1 = PathMap::from_iter(
        pairs
...
fn generic_act_act(bencher: Bencher) {
    use pathmap::{
        PathMap, arena_compact::ArenaCompactTree, morphisms::Catamorphism, zipper::ProductZipperG,
    };
    let mut sink = 0;
    let pairs = read_data();
    let map1 = PathMap::from_iter(
